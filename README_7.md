...
- Every club/national team now has real `primary_color` /
  ...
  Rangers.
- Auto-generated multiple-choice distractors (`buildAutoOptions`) now match
  the "shape" of the correct answer before falling back to any answer in
  the same category. Previously a category like "Club Info" mixes
  founding-year facts, nicknames, and trivia in one answer bank, so a
  "What year was X founded?" question could get non-year wrong options
  (e.g. a nickname or an unrelated club name) — a real example reported
  during this project. A new `answerShape()` helper buckets each answer as
  `year` (4 digits), `number`, or `text`, and distractors are now picked
  same-shape-first, only padding with a different shape if too few
  same-shape candidates exist in that category (and, as before, falling
  back to the global answer bank as a last resort so a question is never
  short of 3 options).
- Fixed two related "pop-up won't close" bugs. First, the sign-in modal
  didn't auto-dismiss after a successful login: `submitAuth()`'s cleanup
  step was unconditionally re-showing the modal every time, even on
  success, instead of only re-showing it when sign-in failed. Second, the
  `&times;` close button on every modal (Sign in, Settings) was a floated
  element that lived inside the modal's own scrollable content — on a
  long modal like Settings, scrolling down to reach a field (e.g. the Log
  out button) scrolled the close button out of view too, so it looked
  like clicking it did nothing; only clicking the darkened area *outside*
  the modal worked, since that's unaffected by internal scroll. The close
  button is now pinned (`position: sticky`) to the top of each modal so
  it's reachable no matter how far you've scrolled inside it.
- Fixed a real login lockout: the underlying cause was trying to
  "sign up" again on an already-registered email (which Supabase silently
  ignores rather than resetting the password), compounded by a client-side
  URL/hash issue that could interfere with the "Forgot password?" email
  recovery link. As an immediate, reliable fix, the affected account's
  password was reset directly at the database level rather than relying
  on the email flow; the recovery-flow race condition may still be worth
